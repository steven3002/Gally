#!/usr/bin/env bash
# =============================================================================
# run_testnet.sh — Devnet E2E orchestration (DEV-M1).
#
# Brings the WHOLE stack up against the OFFICIAL Sui Devnet (not a local node):
#   1. point the sui client at testnet + the funded operator (the recovery-phrase
#      wallet 0x2f35…); NO testnet faucet — gas comes from that wallet (DEV-G1);
#   2. publish usdc(mock) + gally_core + faucet + N entity-token templates to testnet
#      (usdc's mainnet→Circle Published.toml entry is preserved);
#   3. write config.toml with RPC=testnet, the operator key, GAS_SOURCE=operator, ids;
#   4. boot Postgres + the BI indexer pointed at the testnet RPC;
...
#
# Usage:  ./run_testnet.sh [--soak CYCLES] [--users N] [--pool N] [--no-deploy]
# =============================================================================
set -euo pipefail
...
INDEXER_DB="${INDEXER_DB:-gally_testnet}"
...
# ---------- 0. preflight: testnet env + operator + balance ----------
log "0. Preflight — testnet env + operator wallet"
...
[ -n "$CHAIN_ID" ] || die "testnet unreachable at $RPC_URL"
ok "testnet reachable — chain id: $CHAIN_ID"

sui client new-env --alias testnet --rpc "$RPC_URL" >/dev/null 2>&1 || true
...
ok "operator: $OPERATOR_ADDR (testnet)"
...
# ---------- helpers ----------
set_env_testnet() {  # $1 = pkg dir ; STRIP [environments] (testnet is a system env) ; reset Published.toml (keep usdc mainnet)
  python3 - "$1" "$CHAIN_ID" <<'PY'
import sys, re, pathlib
pkg, chain = sys.argv[1], sys.argv[2]
mt = pathlib.Path(pkg, "Move.toml"); t = mt.read_text()
# testnet/mainnet are SYSTEM environments — Sui forbids declaring them in [environments].
# Remove any [environments] block entirely; the active sui client env (testnet) is used.
t = re.sub(r'\n*\[environments\][^\[]*', '\n', t, count=1)
mt.write_text(t.rstrip() + "\n")
# Drop a possibly-corrupt Move.lock (a committed [pinned.testnet] had Windows backslash
# subdirs + a stale framework rev). Sui regenerates it cleanly against the active CLI.
pathlib.Path(pkg, "Move.lock").unlink(missing_ok=True)
...
if [ "$DEPLOY" = 1 ]; then
  ...
  log "1. Publish usdc (mintable mock) → testnet $CHAIN_ID"
  set_env_testnet "$REPO/usdc"
...
PY
  ...
  log "Publish gally_core → testnet"
  set_env_testnet "$REPO/gally_core"
...
PY
  ...
  log "Publish gally_mock_faucet → testnet"
  set_env_testnet "$REPO/gally_mock_faucet"
...
PY
  ...
  log "Publish $ENTITY_POOL_SIZE entity-token templates → testnet"
  ...
  for i in $(seq 1 "$ENTITY_POOL_SIZE"); do
    ...
    bash "$REPO/entity_token_template/scripts/instantiate.sh" "$slug" \
        --out "$out" --name "Gally Entity Token $i" --symbol "GET$i" --description "Testnet entity token $i" --force >/dev/null 2>&1 \
        || die "instantiate et$i failed"
    rm -f "$out/Move.lock"  # avoid the corrupt [pinned.testnet] lock; regenerate clean
...
PY
  ...
  # ---------- 2. testnet config.toml ----------
  log "2. Write config.toml (testnet, operator-funded gas)"
  cat > "$REPO/config.toml" <<EOF
RPC_URL = "$RPC_URL"
FAUCET_URL = "unused-on-testnet"
GAS_SOURCE = "operator"
...
EOF
  ok "config.toml written (testnet ids + GAS_SOURCE=operator)"
else
...
# ---------- 3. publish-only: the systemd indexer + seeding are handled separately ----------
log "3. Publish complete — indexer (systemd) + seeding handled outside this script"
cat <<EOF

  ✅ testnet publish done. New ids written to $REPO/config.toml + sim_state.json:
     chain id        : $CHAIN_ID (testnet)
     GALLY_PACKAGE_ID: ${GALLY_PACKAGE_ID:-?}
     PROTOCOL_CONFIG : ${PROTOCOL_CONFIG_ID:-?}
     ADMIN_CAP       : ${ADMIN_CAP_ID:-?}
     USDC_PACKAGE    : ${USDC_PACKAGE_ID:-?}
     USDC_TREASURY   : ${USDC_TREASURY_CAP_ID:-?}
     FAUCET_PACKAGE  : ${FAUCET_PACKAGE_ID:-?}
     MOCK_FAUCET     : ${MOCK_FAUCET_ID:-?}
     entity tokens   : $ENTITY_POOL_SIZE (sim_state.json)
EOF
exit 0

# ---------- 4. bot: genesis + optional soak ----------
log "4. Build + run the gas-aware simulation bot (testnet)"
