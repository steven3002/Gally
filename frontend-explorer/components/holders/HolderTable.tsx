    ...
    <div className="divide-y divide-border">
      {holders.map((h, i) => {
        ...
        return (
          <div
            key={h.address}
            className="flex flex-col gap-3 px-5 py-3.5 transition-colors hover:bg-surface-2 sm:flex-row sm:items-center sm:justify-between"
          >
            <Link
              href={`/address/${h.address}`}
              className="flex min-w-0 flex-1 items-center gap-3 hover:text-primary"
            >
              <span className="tnum w-5 shrink-0 text-xs font-semibold text-muted-2">{i + 1}</span>
            ...
            <div className="grid grid-cols-3 items-center gap-x-6 sm:flex sm:gap-8 sm:text-right">
              <Cell
                label="Deeds"
              ...
              <div className="min-w-[88px] text-left sm:text-right">
                <div className="text-[11px] text-muted-2">Share</div>
                <div className="tnum text-sm font-semibold text-foreground">{pct(h.pctOfSupply, 2)}</div>
