...
# ---------- 2. deploy ----------
# 2a. Publish the `usdc` package FIRST — gally_core + faucet depend on it for the
# canonical `usdc::usdc::USDC` type. On localnet this is the mintable Mock USDC (its
# `init` hands the TreasuryCap<USDC> to the operator); on mainnet the dependency resolves
# to Circle's published package, whose [published.mainnet] entry we PRESERVE here so the
# gally_core `--build-env mainnet` pre-build still type-checks.
log "2a. Deploy usdc (canonical USDC type provider) → chain $CHAIN_ID"
python3 - "$REPO/usdc" "$CHAIN_ID" <<'PY'
import sys, re, pathlib
pkg, chain = sys.argv[1], sys.argv[2]
mt = pathlib.Path(pkg, "Move.toml"); t = mt.read_text()
block = f'[environments]\nlocalnet = "{chain}"\n'
t = re.sub(r'\[environments\][^\[]*', block + "\n", t, count=1) if "[environments]" in t else t
mt.write_text(t)
# Reset Published.toml to ONLY the static mainnet (Circle) entry — drop any stale localnet
# published-at; the localnet publish below writes a fresh one via automated address mgmt.
CIRCLE = "0xdba34672e30cb065b1f93e3ab55318768fd6fef66c15942c9f7cb846e2f900e7"
pathlib.Path(pkg, "Published.toml").write_text(
  "# Generated by Move\n# This file SHOULD be committed to source control\n\n"
  "[published.mainnet]\nchain-id = \"35834a8a\"\n"
...
print("ok")
PY
publish_pkg "$REPO/usdc" /tmp/rs_usdc.json || die "usdc publish failed"
eval "$(python3 - <<'PY'
import json
d=json.load(open('/tmp/rs_usdc.json')); pkg=cap=None
...
[ -n "${USDC_PACKAGE_ID:-}" ] || die "could not parse usdc ids"
ok "usdc: $USDC_PACKAGE_ID (mintable Mock USDC; TreasuryCap<USDC> $USDC_TREASURY_CAP_ID)"

# 2b. gally_core (depends on usdc for the settlement coin type).
...
eval "$(python3 - <<'PY'
import json
d=json.load(open('/tmp/rs_gally.json')); pkg=None; cfg=adm=None
for c in d.get("objectChanges",[]):
    t=c.get("type"); ot=c.get("objectType","")
    if t=="published": pkg=c["packageId"]
...
ok "gally_core: $GALLY_PACKAGE_ID (config $PROTOCOL_CONFIG_ID, admin $ADMIN_CAP_ID; USDC cap $USDC_TREASURY_CAP_ID from usdc pkg)"
